...
TRIGGER_HELP = {
    ...
    "VOLUME_SPIKE_2X": "Institutional Interest: Today's trading volume is more than double the normal 20-day average.",
...
def find_cached_ticker(company_name, cache_dir):
    ...
    if not os.path.exists(cache_dir): return company_name
    cached_files = [f.replace('.parquet', '') for f in os.listdir(cache_dir) if f.endswith('.parquet')]
    
    if company_name in cached_files: return company_name
...
def parse_broker_file(file_obj):
    ...
    try:
        ...
        t_col = cols.get('instrument') or cols.get('stock name') or cols.get('symbol') or cols.get('company name')
        q_col = cols.get('qty') or cols.get('qty.') or cols.get('quantity') or cols.get('shares')
        p_col = cols.get('avg. cost') or cols.get('average price') or cols.get('avg price') or cols.get('avg. price') or cols.get('average buy price')
        c_col = cols.get('closing price') or cols.get('ltp') or cols.get('current market price') or cols.get('cmp')
...
def metric_card(label, value, note="", state="", tooltip=""):
    ...
    return f'<div class="card"{tooltip_attr}><div class="card-label">{label}</div><div class="card-value {state}">{state_html}{value}</div><div class="card-note">{note}</div></div>'

screener_data = load_screener_data()
updated_at = "Waiting for data"
if screener_data:
    updated_at = datetime.fromisoformat(screener_data.get("updated_at", datetime.now().isoformat())).strftime("%d %b %Y · %I:%M %p")
...
with tab1:
    if not screener_data:
        ...
        trade_ready_count = int((signals_df.get("status") == "Trade-Ready").sum()) if not signals_df.empty else 0
        regime_state = "good" if market_regime == "Bullish" else "bad" if market_regime == "Bearish" else "warn"

        c1,c2,c3,c4 = st.columns(4)
        c1.markdown(metric_card("Market Regime", market_regime.upper(), "Nifty 50 vs 50-day EMA", regime_state, "Shows if the overall market is healthy."), unsafe_allow_html=True)
        c2.markdown(metric_card("Stocks Scanned", f'{screener_data.get("total_scanned",0):,}', "Nifty 500 universe", "", "Total stocks evaluated today."), unsafe_allow_html=True)
        c3.markdown(metric_card("Opportunities", screener_data.get("total_signals",0), "Quant score ≥ 3", "", "Stocks passing strict technical filters."), unsafe_allow_html=True)
        c4.markdown(metric_card("Trade Ready", trade_ready_count, "Quant score 5–6", "good" if trade_ready_count else "", "High-conviction breakout setups."), unsafe_allow_html=True)

        if market_regime == "Bearish":
            st.warning("Market regime is bearish — long setups are filtered while Nifty 50 remains below its 50-day EMA.")

        st.markdown('<div class="section-kicker">Opportunity scanner</div>', unsafe_allow_html=True)
        st.subheader("Signals")

        if not signals_df.empty:
            f1,f2,f3 = st.columns([1.2,1.2,2.6])
            status_filter = f1.multiselect("Status", ["Trade-Ready","Watchlist"], default=["Trade-Ready","Watchlist"])
            min_score = f2.selectbox("Minimum score", [3,4,5,6], index=0)
            search = f3.text_input("Find ticker", placeholder="Search RELIANCE, INFY…")

            filtered = signals_df[signals_df["status"].isin(status_filter) & (signals_df["score"] >= min_score)].copy()
            if search: filtered = filtered[filtered["ticker"].str.contains(search, case=False, na=False)]
            filtered = filtered.sort_values(["score","volume_ratio"], ascending=[False,False])

            col_config = {
                "ticker": st.column_config.TextColumn("Ticker", help="NSE trading symbol."),
                "status": st.column_config.TextColumn("Status", help="Trade-Ready (Score 5-6) or Watchlist (Score 3-4)."),
                "score": st.column_config.ProgressColumn("Score", min_value=0, max_value=6, format="%d / 6", help="Quantitative strength score."),
                "close": st.column_config.NumberColumn("Price", format="₹%.2f", help="Last closing price."),
                "rsi_14": st.column_config.NumberColumn("RSI", format="%.1f", help="Momentum indicator (60-70 is the sweet spot)."),
                "volume_ratio": st.column_config.NumberColumn("Volume", format="%.2fx", help="Today's volume compared to the 20-day average."),
                "pct_above_50": st.column_config.NumberColumn("Above 50 EMA", format="%+.2f%%", help="Distance from 50-day average."),
                "triggers_str": st.column_config.TextColumn("Technical triggers", help="The rules the stock passed today."),
            }
            display_cols = ["ticker","status","score","close","rsi_14","volume_ratio","pct_above_50","triggers_str"]
            st.dataframe(filtered[display_cols], column_config=col_config, hide_index=True, use_container_width=True, height=min(500, 74 + 35*max(len(filtered),1)))

            st.markdown('<div class="section-kicker">Stock analysis</div>', unsafe_allow_html=True)
            a,b = st.columns([2,1])
            selected_stock = a.selectbox("Inspect stock", options=filtered["ticker"].tolist() if not filtered.empty else signals_df["ticker"].tolist())
            period = b.segmented_control("Chart range", options=["3M","6M","1Y"], default="6M")

            row = signals_df.loc[signals_df["ticker"] == selected_stock].iloc[0]
            status_class = "good" if row.get("status") == "Trade-Ready" else "warn"
            triggers = row.get("triggers", [])
            trigger_html = "".join(f'<span class="pill" title="{TRIGGER_HELP.get(t, t)}" style="cursor:help;">✓ {t}</span>' for t in triggers)
            st.markdown(f'''<div class="stock-head"><div class="stock-title">{selected_stock} <span class="{status_class}" style="font-size:.8rem;margin-left:.5rem">● {row.get("status","")}</span></div><div class="stock-meta">₹{row.get("close",0):,.2f} &nbsp; · &nbsp; Score {int(row.get("score",0))}/6 &nbsp; · &nbsp; RSI {row.get("rsi_14",0):.1f} &nbsp; · &nbsp; Volume {row.get("volume_ratio",0):.2f}× &nbsp; · &nbsp; {row.get("pct_above_50",0):+.2f}% above 50 EMA</div><div style="margin-top:.65rem">{trigger_html}</div></div>''', unsafe_allow_html=True)
            ...
            if ohlcv_df is not None:
                ...
                fig = go.Figure()
                fig.add_trace(go.Candlestick(x=chart_df.index, open=chart_df["Open"], high=chart_df["High"], low=chart_df["Low"], close=chart_df["Close"], name="Price", increasing_line_color="#38d996", decreasing_line_color="#ff6470"))
                fig.add_trace(go.Scatter(x=chart_df.index, y=chart_df["EMA_50"], mode="lines", name="50 EMA", line=dict(color="#f2b84b", width=1.4)))
                fig.add_trace(go.Scatter(x=chart_df.index, y=chart_df["EMA_200"], mode="lines", name="200 EMA", line=dict(color="#6aa9ff", width=1.6)))
                fig.update_layout(height=560, margin=dict(l=10,r=10,t=20,b=10), paper_bgcolor="#0b0f14", plot_bgcolor="#0b0f14", font=dict(color="#aeb9c4"), xaxis_rangeslider_visible=False, hovermode="x unified", legend=dict(orientation="h", y=1.03, x=0), xaxis=dict(showgrid=False), yaxis=dict(title="Price (INR)", gridcolor="#1b2530"))
                ...
                st.markdown('<div class="section-kicker" style="margin-top: 1.5rem;">Trade Execution Guide</div>', unsafe_allow_html=True)
                st.markdown(f'''
...
# ==========================================
with tab2:
    st.markdown('<div class="section-kicker">Portfolio intelligence</div>', unsafe_allow_html=True)
    st.header("Portfolio Health Analyzer")
    
    st.info("🔒 **Privacy First:** Upload your Groww (or standard broker) Holdings CSV or Excel file here. It is processed 100% in your browser memory and never saved.")
    
    uploaded_file = st.file_uploader("Upload Holdings (CSV or XLSX)", type=["csv", "xlsx"])
    
    if uploaded_file is not None:
        port_df, err = parse_broker_file(uploaded_file)
        
        if port_df is None or port_df.empty:
            st.error(f"Failed to parse file. Ensure it has Ticker/Stock Name, Quantity, and Average Price columns. Details: {err}")
        else:
            port_stats = []
            for _, row in port_df.iterrows():
                ...
                stats = {
                    "Stock": company_name,
                    "Matched Symbol": matched_ticker if matched_ticker != company_name else "Not in Cache",
                    "Quantity": qty,
                    "Buy Price": avg_price,
            ...
            col_conf = {
                "Stock": "Stock Name",
                "Matched Symbol": st.column_config.TextColumn("Ticker", help="The NSE symbol matched in your local Nifty 500 database."),
            ...
            if not breakdowns.empty:
                st.error(f"🚨 **Action Required:** {len(breakdowns)} of your holdings have broken below their 200-day EMA. These are mathematically in a long-term downtrend.")
                st.dataframe(breakdowns, column_config=col_conf, hide_index=True, use_container_width=True)
            else:
                st.success("✅ Excellent! None of your recognized holdings are trading below their 200-day EMA.")

            st.divider()

            signals_df = pd.DataFrame(screener_data.get("signals", [])) if screener_data else pd.DataFrame()
            if not signals_df.empty:
                bullish_held = signals_df[signals_df["ticker"].isin(res_df["Matched Symbol"].tolist())]
                st.subheader("🔥 Active Screener Setups in Your Portfolio")
                if not bullish_held.empty:
                    st.write("These stocks you currently own are triggering fresh quantitative breakout signals today:")
                    st.dataframe(
                    ...
                    st.info("None of your current holdings triggered a new screener setup today.")

            st.divider()
            
            st.subheader("All Parsed Holdings")
            st.dataframe(res_df, column_config=col_conf, hide_index=True, use_container_width=True)

# ==========================================
# TAB 3: TA GLOSSARY (Educational)
# ==========================================
with tab3:
    st.markdown('<div class="section-kicker">Educational Reference</div>', unsafe_allow_html=True)
    st.header("Technical Analysis (TA) Glossary")
    st.write("A beginner-friendly guide to understanding the quantitative metrics used in this screener.")
